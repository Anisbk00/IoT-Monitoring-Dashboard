// =====================================================================
// IoT Monitoring — ESP32 + DHT11 + MQ-2 Production Firmware
// Hardware: ESP32 + DHT11 (temp/humidity) + MQ-2 (gas/smoke/CO2)
// =====================================================================
...
//   3. From a phone/computer, connect to WiFi "IoT-ESP32-XXXX"
...
//   7. Configure WiFi/MQTT on the ESP32's AP page or via dashboard
//   8. ESP32 connects to WiFi, starts sending sensor data via MQTT
//
// WIRING:
...
#define MQ2_PIN       34    // GPIO34 (ADC1_CH6) -> MQ-2 analog
#define MQ2_DIG_PIN   35    // GPIO35 -> MQ-2 digital out

// === MQ-2 Calibration =================================================
const float MQ2_RL         = 5.0;    // Load resistance (kΩ)
const float MQ2_RO_CLEAN   = 9.83;   // Ro in clean air (calibrate YOUR sensor!)
const int   MQ2_WARMUP_MS  = 30000;  // 30s warm-up time
const int   MQ2_SMOOTH_N   = 5;      // Smoothing samples
...
bool dhtOk = false, mq2Ok = false;
int  dhtFailCount = 0, mq2FailCount = 0;
int  mq2Samples[MQ2_SMOOTH_N];
int  mq2SampleIdx = 0;
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .badge-amber{background:#f59e0b33;color:#f59e0b}
  ...
  .wifi-scan{margin-top:8px}
  .wifi-list{max-height:150px;overflow-y:auto;margin-top:8px}
  .wifi-item{padding:8px 12px;border:1px solid #334155;border-radius:8px;margin-bottom:4px;
    cursor:pointer;transition:background .2s;display:flex;justify-content:space-between;align-items:center}
  .wifi-item:hover{background:#334155}
  .wifi-item.selected{border-color:#10b981;background:#10b98111}
  .wifi-ssid{font-size:0.85em}
  .wifi-rssi{font-size:0.7em;color:#64748b}
  .wifi-lock{font-size:0.7em;color:#f59e0b}
  @media print{
...
<div class="container">
  ...
  <div class="card qr-card">
    ...
    <ol class="steps">
      ...
      <li>Configure WiFi &amp; MQTT below (or on the dashboard)</li>
      <li>The device will connect and start sending data!</li>
    </ol>
  </div>
  ...
  <div class="card">
    ...
    <div class="info-row"><span class="info-label">Sensors</span><span class="info-value">DHT11 + MQ-2</span></div>
    <div class="info-row"><span class="info-label">Status</span><span class="badge badge-blue">Waiting for setup</span></div>
  </div>

  <div class="card">
    <h2>⚙️ WiFi &amp; MQTT Setup <span class="badge badge-amber">Required</span></h2>
    <p style="color:#64748b;font-size:0.8em;margin-bottom:14px">
      Configure your WiFi network and MQTT broker to start sending data:
    </p>
    <form id="setupForm" onsubmit="return sendConfig(event)">
      <div class="field">
        ...
        <div style="position:relative">
          ...
          <button type="button" onclick="scanWifi()" style="position:absolute;right:4px;top:50%;transform:translateY(-50%);
            background:transparent;border:none;color:#10b981;font-size:0.75em;cursor:pointer;padding:4px 8px">Scan</button>
        </div>
      ...
      <div class="field">
        ...
        <input id="broker" placeholder="e.g. 192.168.1.100" required>
      </div>
      <div class="field">
        <label>MQTT Port</label>
        <input id="mqttPort" type="number" value="1883" placeholder="1883">
      </div>
    ...
    <div id="result" style="display:none" class="success">
      ...
      <p style="margin-top:12px;font-size:0.8em">The device will appear as <strong>"Online"</strong> in your dashboard once connected.</p>
    </div>
  </div>
</div>
...
function scanWifi(){
  ...
  list.innerHTML='<div style="padding:8px;color:#94a3b8;font-size:0.8em">Scanning...</div>';
  fetch('/scan').then(r=>r.json()).then(networks=>{
    list.innerHTML='';
    networks.forEach(function(n){
      var div=document.createElement('div');
      div.className='wifi-item';
      div.innerHTML='<span class="wifi-ssid">'+n.ssid+'</span><span>'+(n.rssi>-50?'📶':n.rssi>-70?'📶':'📶')+' '+n.rssi+'dBm'+(n.secure?' <span class="wifi-lock">🔒</span>':'')+'</span>';
    ...
    if(networks.length===0) list.innerHTML='<div style="padding:8px;color:#94a3b8;font-size:0.8em">No networks found</div>';
  }).catch(()=>{list.innerHTML='<div style="padding:8px;color:#f87171;font-size:0.8em">Scan failed</div>';});
}
...
async function sendConfig(e){
  ...
  var data={
    ...
    mqttPort:parseInt(document.getElementById('mqttPort').value)||1883,mqttWsPort:3003
  };
  try{
    ...
    else{var err=await r.json();alert('Failed: '+(err.message||'Invalid config'));btn.disabled=false;btn.textContent='💾 Save & Connect'}
  ...
  // GET /scan — WiFi network scan for the captive portal
...
void waitForSerialProvisioning() {
  Serial.println();
  Serial.println("========================================");
  Serial.println("  IoT Monitor - ESP32 + DHT11 + MQ-2");
  Serial.println("========================================");
...
int readMQ2Raw() { return analogRead(MQ2_PIN); }

float getMQ2Smoothed() {
  long sum = 0;
  for (int i = 0; i < MQ2_SMOOTH_N; i++) sum += mq2Samples[i];
  return (float)sum / MQ2_SMOOTH_N;
}

float calculatePPM(float smoothedRaw) {
  if (smoothedRaw <= 0) return 0;
  float vrl = (smoothedRaw / 4095.0) * 3.3;
  float rs  = (3.3 - vrl) / vrl * MQ2_RL;
  float ratio = rs / MQ2_RO_CLEAN;
  if (ratio <= 0) return 0;
  // MQ-2 LPG curve: ppm = (ratio / 0.65)^(1/-0.45)
  return max(0.0f, (float)pow(ratio / 0.65, 1.0 / -0.45));
}
...
void initSensors() {
  ...
  for (int i = 0; i < MQ2_SMOOTH_N; i++)
    mq2Samples[i] = analogRead(MQ2_PIN);
  mq2Ok = true; mq2FailCount = 0;
  Serial.println("[MQ-2]  OK on GPIO" + String(MQ2_PIN));
  Serial.println("[MQ-2]  Warming up (30s)...");
...
void checkSensorHealth() {
  ...
  if (!mq2Ok && mq2FailCount > 0 && mq2FailCount % 6 == 0) {
    int v = analogRead(MQ2_PIN);
    if (v > 0 && v < 4095) {
      mq2Ok = true; mq2FailCount = 0;
      Serial.println("[MQ-2] RECOVERED!");
      publishStatus("sensor_recovered", "MQ-2");
    }
  }
}
...
void sendSensorData() {
  ...
  int gasRaw = -1; float gasPPM = -1;

  // DHT11: range 0-50°C (±2°C), humidity 20-90% (±5%)
  if (dhtOk) {
  ...
  // MQ-2 gas sensor with warm-up and smoothing
  if (mq2Ok && mq2WarmedUp) {
    mq2Samples[mq2SampleIdx] = readMQ2Raw();
    mq2SampleIdx = (mq2SampleIdx + 1) % MQ2_SMOOTH_N;
    gasRaw = (int)getMQ2Smoothed();
    gasPPM = calculatePPM(getMQ2Smoothed());
    if (gasRaw == 0 || gasRaw >= 4090) {
      mq2FailCount++;
      if (mq2FailCount >= 5) {
        mq2Ok = false;
        Serial.println("[MQ-2] FAILED 5x -> offline");
        publishStatus("sensor_error", "MQ-2 not responding");
      }
    } else { mq2FailCount = 0; }
  } else if (mq2Ok && !mq2WarmedUp) {
    mq2Samples[mq2SampleIdx] = readMQ2Raw();
    mq2SampleIdx = (mq2SampleIdx + 1) % MQ2_SMOOTH_N;
  }
  ...
  payload += "\"co2\":" + (gasPPM >= 0 ? String((int)gasPPM) : "null") + ",";
  payload += "\"humidity\":" + (!isnan(humidity) ? String(humidity,1) : "null") + ",";
  payload += "\"gasRaw\":" + (gasRaw >= 0 ? String(gasRaw) : "null");
  payload += "}";
...
void setup() {
  ...
  if (loadConfig()) {
    ...
    if (mqtt.connected()) {
      String status = dhtOk && mq2Ok ? "online" : "sensor_error";
...
void loop() {
  ...
  if (Serial.available()) {
    ...
    if (input == "RESET") {
      ...
      Serial.println("MQ-2: " + String(mq2Ok ? "OK" : "FAIL"));
  ...
  if (!mq2WarmedUp && (millis() - bootTime >= MQ2_WARMUP_MS)) {
    mq2WarmedUp = true;
