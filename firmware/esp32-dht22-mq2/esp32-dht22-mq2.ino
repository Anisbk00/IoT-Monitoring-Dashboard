// =====================================================================
// IoT Monitoring — ESP32 Production Firmware
// Hardware: ESP32 + DHT22 (temp/humidity) + MQ-2 (gas/smoke)
// =====================================================================
...
//   5. Print the QR sticker and stick it on the ESP32 enclosure
//   6. In the dashboard app, click "Add Device" → scan the QR code
//   7. Fill in device name, location → Register!
//   8. ESP32 can also be configured directly via the AP page form
//
// WIRING:
//   DHT22:  VCC->3.3V, GND->GND, DATA->GPIO4 (with 10k pull-up to 3.3V)
...
const float MQ2_RO_CLEAN   = 9.83;  // Calibrate in clean air!
const int   MQ2_WARMUP_MS  = 30000;
const int   MQ2_SMOOTH_N   = 5;
...
<script>
// Mini QR code generator - generates QR matrix in JS
// Based on nayuki/QR-Code-generator (simplified for ESP32 AP page)
...
(function(){
  ...
  // We'll create the QR matrix manually for small payloads
  // For the ESP32, we use a pure-JS QR encoder

  // Simple QR code drawing using external-free algorithm
  // Generate QR code version 2-6 (25-77 modules) for our small payload
  try {
...
void waitForSerialProvisioning() {
  Serial.println();
  Serial.println("========================================");
  Serial.println("  IoT Monitor - ESP32 Setup");
  Serial.println("========================================");
  ...
  String input = "";
  while (!provisioned) {
    apServer.handleClient();  // Keep AP web server alive
    ...
    while (Serial.available()) {
      ...
      if (c == '\n') {
        ...
        if (input.startsWith("PROVISION ")) {
          ...
          // Try as a provisioning payload if it has the right type
...
void initSensors() {
  ...
  Serial.println("[DHT22] OK on GPIO" + String(DHT_PIN));
...
void checkSensorHealth() {
  if (!dhtOk && dhtFailCount > 0 && dhtFailCount % 6 == 0) {
    ...
    if (!isnan(t)) {
      ...
      publishStatus("sensor_recovered", "DHT22");
    }
  }
...
void sendSensorData() {
  ...
  if (dhtOk) {
    ...
    if (isnan(temperature) || isnan(humidity)) {
      ...
      if (dhtFailCount >= 3) {
        dhtOk = false;
        Serial.println("[DHT22] FAILED 3x -> offline");
        publishStatus("sensor_error", "DHT22 not responding");
      }
    } else { dhtFailCount = 0; }
  }
...
void loop() {
  if (!provisioned) {
    // In AP mode, waiting for provisioning
